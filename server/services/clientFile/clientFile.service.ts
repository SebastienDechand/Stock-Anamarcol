...
export function findClientFileById(id: string) {
  return ClientFileModel.findById(id).populate('contactRef', 'name email phone').lean();
}

export function findClientFileDocument(id: string): Promise<HydratedDocument<IClientFile> | null> {
