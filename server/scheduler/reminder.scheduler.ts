...
export function startReminderScheduler(): void {
  ...
  cron.schedule(
    '0 8 * * *',
    async () => {
      console.log('[Scheduler] Starting vehicle reminders check...');
      try {
        const reminders = await checkAndSendVehicleReminders();
        if (reminders.length > 0) {
          console.log(`[Scheduler] ${reminders.length} reminder(s) sent successfully`);
        } else {
          console.log('[Scheduler] No reminders to send today');
        }
      } catch (error) {
        console.error('[Scheduler] Error while checking reminders:', error);
      }
    },
    {
  ...
  console.log('[Scheduler] Vehicle reminders enabled (every day at 08:00 CET)');
}
