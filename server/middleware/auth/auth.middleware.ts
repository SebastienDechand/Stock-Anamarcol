...
function requireRole(roles: Role[], accessDenied?: AccessDenied) {
  return (req: Request, res: Response, next: NextFunction): void => {
    const token = req.cookies.jwt;
    if (!token) {
      res.status(401).json({ message: 'Authentication required', code: ErrorCode.AUTH_REQUIRED });
      return;
    }
    ...
    resolveUser(token)
      .then((user) => {
        if (!user) {
          res.status(401).json({ message: 'User not found', code: ErrorCode.USER_NOT_FOUND });
          return;
        }
        if (accessDenied && !roles.some((role) => user.roles?.includes(role))) {
...
export const requireHotline = requireRole([Role.HOTLINE, Role.ADMIN, Role.SUPERADMIN], {
...
export const requireMonteur = requireRole([Role.MONTEUR, Role.ADMIN, Role.SUPERADMIN], {
