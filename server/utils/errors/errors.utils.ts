...
export const signUpErrors = (err: MongoError): SignUpError => {
  ...
  if (err.message.includes('username')) errors.username = 'Invalid or already taken username';
  if (err.message.includes('email')) errors.email = 'Invalid email';
  if (err.message.includes('password')) errors.password = 'Password must be at least 6 characters';
...
export const createItemErrors = (err: Error): CreateItemError => {
  ...
  if (err.message.includes('name')) errors.name = 'Invalid or already taken name';
  if (err.message.includes('supplier')) errors.supplier = 'Enter a valid supplier';
  if (err.message.includes('status')) errors.status = 'Status must be NEW or RMA';
  if (err.message.includes('quantity')) errors.quantity = 'Quantity must be a number';
