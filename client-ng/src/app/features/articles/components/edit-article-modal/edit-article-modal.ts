...
@Component({
  ...
  templateUrl: './edit-article-modal.html',
  styleUrl: './edit-article-modal.scss',
})
export class EditArticleModal implements OnInit, OnDestroy {
  ...
  submit() {
    if (!this.form.denomination.trim() || !this.form.fournisseur || !this.form.etat) {
      this.error = this.translate.instant('ARTICLES.REQUIRED_FIELDS');
      return;
    }
  ...
  onFileChange(event: Event) {
    ...
    if (!ACCEPTED_IMAGE_TYPES.includes(file.type)) {
      this.fileError = this.translate.instant('ARTICLES.IMAGE_FORMAT_ERROR');
      return;
    }
    if (file.size > MAX_FILE_SIZE) {
      this.fileError = this.translate.instant('ARTICLES.IMAGE_SIZE_ERROR');
      return;
    }
  ...
  readonly ACTION_LABEL_KEYS: Record<string, string> = {
    create: 'ARTICLES.ACTION_CREATE',
    update: 'ARTICLES.ACTION_UPDATE',
    delete: 'ARTICLES.ACTION_DELETE',
    quantity_change: 'ARTICLES.QUANTITY',
  };
  ...
  readonly FIELD_LABEL_KEYS: Record<string, string> = {
    denomination: 'ARTICLES.DENOMINATION',
    fournisseur: 'ARTICLES.SUPPLIER',
    etat: 'ARTICLES.STATE',
    quantite: 'ARTICLES.QUANTITY',
    prepaCG: 'ARTICLES.PREPA_CG',
    prepaTPV: 'ARTICLES.PREPA_TPV',
