    ...
    this.actions$.pipe(
      ...
      exhaustMap(({ data }) =>
        this.api.post<{ item: Item }>('api/item/', data).pipe(
          map((response) => {
            this.toast.success('TOAST.ARTICLE_ADDED');
            return ItemsActions.createItemSuccess({ item: response.item });
          }),
          catchError((error) => {
            this.toast.error('TOAST.ARTICLE_ADD_ERROR');
    ...
    this.actions$.pipe(
      ...
      exhaustMap(({ id, data }) =>
        this.api.put<{ item: Item }>(`api/item/${id}`, data).pipe(
          map((response) => {
            this.toast.success('TOAST.ARTICLE_UPDATED');
            return ItemsActions.updateItemSuccess({ item: response.item });
          }),
          catchError((error) => {
            this.toast.error('TOAST.ARTICLE_UPDATE_ERROR');
    ...
    this.actions$.pipe(
      ...
      exhaustMap(({ id }) =>
        this.api.delete<void>(`api/item/${id}`).pipe(
          map(() => {
            this.toast.success('TOAST.ARTICLE_DELETED');
            return ItemsActions.deleteItemSuccess({ id });
          }),
          catchError((error) => {
            this.toast.error('TOAST.ARTICLE_DELETE_ERROR');
    ...
    this.actions$.pipe(
      ...
      exhaustMap(({ id, quantite, modifierName, operation }) =>
        this.api.put<{ item: Item }>(`api/item/${id}`, { quantite, modifierName, operation }).pipe(
          map((response) => {
            this.toast.success('TOAST.ARTICLE_QTY_UPDATED');
    ...
    this.actions$.pipe(
      ...
      exhaustMap(({ id, formData }) => {
        ...
        return this.api.postFormData<Item>('api/item/upload', formData).pipe(
          map((item) => {
            this.toast.success('TOAST.ARTICLE_IMAGE_UPDATED');
            return ItemsActions.uploadItemPictureSuccess({ item });
          }),
          catchError((error) => {
            this.toast.error('TOAST.ARTICLE_IMAGE_ERROR');
          ...
          .pipe(
            switchMap((response) => {
              const key =
                field === 'prepaCG'
                  ? operation === 'increment'
                    ? 'ARTICLES.PREPA_BATCH_CG_INCREMENTED'
                    : 'ARTICLES.PREPA_BATCH_CG_DECREMENTED'
                  : operation === 'increment'
                    ? 'ARTICLES.PREPA_BATCH_TPV_INCREMENTED'
                    : 'ARTICLES.PREPA_BATCH_TPV_DECREMENTED';
              this.toast.success(key, { count: response.updated });
              return of(ItemsActions.prepaBatchSuccess(), ItemsActions.fetchItems({ params }));
            }),
            catchError(() => {
              this.toast.error('TOAST.ARTICLE_BATCH_ERROR');
