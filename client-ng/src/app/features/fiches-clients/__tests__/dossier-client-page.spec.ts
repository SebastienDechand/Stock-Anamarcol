...
import { initialRapportsState } from '../../rapports-intervention/store/rapports.state';
...
describe('DossierClientPage', () => {
  let component: DossierClientPage;
  let filesFacade: ClientFilesFacade;
  let rapportsFacade: RapportsFacade;

  beforeEach(async () => {
    TestBed.overrideComponent(DossierClientPage, { set: { template: '', imports: [] } });

    await TestBed.configureTestingModule({
      imports: [DossierClientPage],
      providers: [
    ...
    rapportsFacade = TestBed.inject(RapportsFacade);
    vi.spyOn(filesFacade, 'loadOne').mockImplementation(() => {});
    vi.spyOn(rapportsFacade, 'loadByClientFile').mockImplementation(() => {});

    const fixture = TestBed.createComponent(DossierClientPage);
  ...
  it('ngOnInit calls filesFacade.loadOne and rapportsFacade.loadByClientFile with the route id', () => {
    expect(filesFacade.loadOne).toHaveBeenCalledWith('file-id-1');
    expect(rapportsFacade.loadByClientFile).toHaveBeenCalledWith('file-id-1');
  });
  ...
  describe('onWizardSaved()', () => {
    ...
    it('calls rapportsFacade.loadByClientFile with the current fileId', () => {
      vi.mocked(rapportsFacade.loadByClientFile).mockClear();
  ...
  describe('confirmDeleteReport()', () => {
    it('calls rapportsFacade.delete with the deleteReportId and clears the signal', () => {
      vi.spyOn(rapportsFacade, 'delete').mockImplementation(() => {});
      component.deleteReportId.set('report-99');

      component.confirmDeleteReport();

      expect(rapportsFacade.delete).toHaveBeenCalledWith('report-99');
      expect(component.deleteReportId()).toBeNull();
    });

    it('does NOT call rapportsFacade.delete when deleteReportId is null', () => {
      vi.spyOn(rapportsFacade, 'delete').mockImplementation(() => {});
      component.deleteReportId.set(null);

      component.confirmDeleteReport();

      expect(rapportsFacade.delete).not.toHaveBeenCalled();
    });
  });
