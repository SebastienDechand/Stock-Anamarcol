...
import { RapportsActions } from '../../../rapports-intervention/store/rapports.actions';
import {
  CashguardUnit,
  InterventionReport,
...
export class ReportWizardModal implements OnChanges {
  ...
  private facade = inject(RapportsFacade);
  ...
  submit(): void {
    ...
    if (this.existing) {
      this.actions$
        .pipe(
          ofType(RapportsActions.updateRapportSuccess, RapportsActions.updateRapportFailure),
          take(1),
          takeUntilDestroyed(this.destroyRef),
        )
        .subscribe((action) => {
          this.loading.set(false);
          if (action.type === RapportsActions.updateRapportSuccess.type) this.saved.emit();
        });
      this.facade.update(this.existing._id, data);
    } else {
      this.actions$
        .pipe(
          ofType(RapportsActions.createRapportSuccess, RapportsActions.createRapportFailure),
          take(1),
          takeUntilDestroyed(this.destroyRef),
        )
        .subscribe((action) => {
          this.loading.set(false);
          if (action.type === RapportsActions.createRapportSuccess.type) this.saved.emit();
        });
