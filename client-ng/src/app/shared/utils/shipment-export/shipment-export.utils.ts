...
import { downloadBlob } from '../download/download.utils';

function toRows(shipments: Shipment[]) {
  return shipments.map((shipment) => ({
    Statut: shipment.sent ? 'Envoyé' : 'En attente',
    Nom: shipment.lastName,
    Prénom: shipment.firstName,
    Société: shipment.company,
    'Société / Fonction': shipment.companyOrRole,
    Pièce: shipment.part,
    Adresse: shipment.address,
    CP: shipment.postalCode,
    Ville: shipment.city,
    Tél: shipment.phone || '',
    'Tél 2': shipment.phone2 || '',
