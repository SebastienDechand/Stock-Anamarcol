...
🚧 En cours de migration — remplace progressivement `client/` (React). Non déployée en production.
...
| ----------------------------- | ---------------------- | :------------: | ----------------------------------------------- |
| `/` *(guest)*                  | Login                   |  `guestGuard`  | Connexion — redirige si déjà authentifié        |
...
Testée dans `.github/workflows/ci.yml` (job `test-client-ng`, en parallèle des tests React et backend). Non incluse dans `deploy.yml` — pas encore déployée.
