...
import type { ItemState, User } from "../../types";
...
const ItemModale = ({ onClose }: ItemModaleProps) => {
  ...
  const { selectedItemInfo, selectedItemQuantite } = useSelector(
    (state: { itemReducer: ItemState }) => state.itemReducer,
  );
  ...
  const handleUpdate = async () => {
    try {
      if (selectedItemInfo?._id && modifierName) {
        await dispatch(
  ...
  useEffect(() => {
    if (selectedItemInfo?.quantite !== undefined) {
      ...
      selectedItemQuantite !== undefined &&
